...
const purseDetails = ref<Array<{ id: string, amount: number, eventName: string }>>([])

const fetchPurseDetails = async (purseId: string) => {
  try {
    const { data } = await useAPI(`/user-management/get-my-guaranteed-amounts`, {
      default: () => [],
      query: {
...
<template>
  <SheetHeader class="justify-between">   
    <SheetTitle class="text-[20px] font-[700] text-[#152A3C]">
      {{ title }} 
    </SheetTitle>
  ...
  <div class="flex-grow overflow-y-auto no-scrollbar flex flex-col">
    ...
    <form class="flex flex-col gap-6 flex-grow p-6 gap-y-[24px]" @submit.prevent> 
        <div class="p-[8px] rounded-[4px] bg-[#F3F8FC] leading-[20px] text-[#152A3C] font-[400] text-[14px]">
            <p>Eventos con garantía en curso</p>
        </div>
        <ul >
          <li v-for="item in purseDetails" :key="item.id" class="flex flex-col ">
            <div class="flex flex-col gap-y-[4px]">
              ...
              <button @click="" class="flex text-[#F97316] font-[400] text-[14px]">Ver evento</button>
            </div>
          </li>
        </ul>
    </form>

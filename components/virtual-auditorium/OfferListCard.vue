...
<template>
  <Countdown :key="key" v-slot="{ days, hours, minutes, seconds, totalSeconds }" :time="endMiliseconds">
    <div tabindex="0" :class="cn(
      ...
      <div class="bg-[#C7E0F0] text-white text-sm font-medium text-center py-2">
        <div class="flex items-center space-x-2 justify-center text-[#152A3C] text-xs font-semibold px-2">
      ...
      <div class="px-2 py-2">
        ...
        <div class="flex items-center justify-between space-x-2 text-[#68686C] text-xs font-semibold mb-2">
          ...
          <span class="font-bold text-sm text-[#20445E]">USD ${{ offer.bids[0]?.amount }}</span>
        </div>
        <div class="flex justify-end">
          <button class="px-4 py-1 text-sm text-[#388EBF] font-medium hover:text-white hover:bg-bluePrimary rounded-md"

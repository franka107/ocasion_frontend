<template>
  <section>
    <ContentLayout title="Listado de sustentos de transferencia y entrega">
      <CustomSimpleCard title="Sustentos de transferencia y entrega" class="mb-6"
          ...
          <CustomTable 
            ...
            <template #SupportForTransfer="{ row }">
              <div 
               ...
               :class="!row.transferenceSupport?.files.length ? 'cursor-not-allowed' : 'cursor-pointer'"
               @click="() => { if(row.transferenceSupport?.files?.length) openSupportForTransferModal(row) }">
                <CustomIcons 
                  :name="transferSupportIcons.get(row.transferenceSupport?.status)?.icon || 'Doc-Loupe'"
                  :class="transferSupportIcons.get(row.transferenceSupport?.status)?.class || 'text-[#AFAFB1]'" />
              </div>
            </template>
            <template #SupportForDelivery="{ row }">
              <div 
               class="flex items-center justify-center" 
              :class="!row.deliverySupport?.files.length ? 'cursor-not-allowed' : 'cursor-pointer'"
              @click="() => { if(row.deliverySupport?.files?.length) openDeliveryForTransferModal(row) }">
                <CustomIcons 
                  :name="deliverySupportIcons.get(row.deliverySupport?.status)?.icon || 'Doc-Transfer'"
                  :class="deliverySupportIcons.get(row.deliverySupport?.status)?.class || 'text-[#AFAFB1]'" />
              </div>
            </template>
            <template #status="{ row }">
              <CustomChip :text="evidenceStatus.get(row.status)?.name || ''"
                :variant="evidenceStatus.get(row.status)?.color as any"></CustomChip>
            </template>
          </CustomTable>
          <SheetContent v-model:open="openModalDetailSupport" class="flex flex-col h-full"
            @pointer-down-outside="(e) => e.preventDefault()" @interact-outside="(e) => e.preventDefault()">
            <SupportForm 
              :id="supportId" 
              :onConfirm="handleConfirmTransference" 
              :onEdit="handleEditTransference"
              :closeModal="() => openModalDetailSupport = false" />
          </SheetContent>
          <SheetContent v-model:open="openModalDeliveryTransfer" class="flex flex-col h-full"
            @pointer-down-outside="(e) => e.preventDefault()" @interact-outside="(e) => e.preventDefault()">
            <DeliveryForm 
              :id="deliveryId" 
...
import { evidenceStatus, evidenceSearch, evidenceHeader, deliverySupportIcons, transferSupportIcons } from "~/constants/evidenceOrg";
...
const { confirmTransference, sortOptions, page, onSort } = useEvidenceTransferAPI()
...
const handleConfirmTransference = async (value: { transferenceSupportId: string }) => {
  openConfirmModal({
...
const handleEditTransference = async (values: any) => {
  openConfirmModal({
    ...
    message: "¿Estás seguro de que deseas actualizar esta Transferencia de bienes?",
    callback: async () => {
      const { status, error }: any = await editDeliverySupport(values);
      if (status.value === "success") {
        openModalDeliveryTransfer.value = false;

<template>
          ...
          <div class="shadow-md rounded-lg px-6 bg-white flex-grow mb-auto">
            ...
            <template #SupportForDelivery="{ row }">
                <div 
                  ...
                  <CustomIcons 
                    :name="deliverySupportIcons.get(row.deliverySupport?.status)?.icon || 'Doc-Loupe'"
                    :class="deliverySupportIcons.get(row.deliverySupport?.status)?.class || 'text-[#AFAFB1]'" />
                </div>
              </template>
              <template #status="{ row }">
                <CustomChip :text="evidencePlatfStatus.get(row.status)?.name || ''"
                  :variant="evidencePlatfStatus.get(row.status)?.color as any"></CustomChip>
              </template>
            </CustomTable>
  ...
  import { evidencePlatfStatus, evidencePlatfSearch, evidencePlatfHeader, deliverySupportIcons } from "~/constants/evidencePlatf";
  ...
  const { confirmDeliverySupport, editDeliverySupport, sortOptions, page, onSort } = useEvidenceDeliveryAPI()

...
try {
  const { data } = await useAPI<DeliveryDetail>(
    `${BASE_EVIDENCE_URL}/get-delivery-aqui-support-detail`,
    {
      query: { id: props.id },
    } as any,
  )
...
<template>
  <SheetHeader>
    ...
    <SheetTitle class="text-xl font-medium text-[#64748B]"
      >Detalle sustento de entrega</SheetTitle
    >
  </SheetHeader>
          ...
          <section class="mb-6">
            ...
            <template v-if="currentMode === 'confirm'">
              <div class="flex items-center mb-[8px]">
                ...
                <div
                  class="font-[700] text-[#20445E] text-[14px] tracking-[0.2px] ml-[24px]"
                >
                  HORA:
                </div>
                <div

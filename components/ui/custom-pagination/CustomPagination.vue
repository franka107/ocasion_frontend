<template>
    <div class="w-full flex justify-between">
        <div class="flex flex-row items-center">
            ...
            <div class="ml-2  bg-white rounded-[8px] text-primary w-[34px] h-[36px] flex justify-center items-center">{{ total }}</div>
        </div>
        <div class="flex flex-row gap-x-[6px]">
            <button
                @click="props.page > 1 ? emit('update:page', props.page - 1) : () => {}"
                :class="activeBtnClass({ size: 'md' })"
            >
                <CustomIcons name="ArrowLeft" class="rotate-180 mr-1 w-6 h-6"/><span>Back</span>
            </button>
            <button
                v-for="(pageNumber,i) in  pages"
                :key="i"
                :class="activeBtnClass({ status: props.page === pageNumber ? 'active': 'inactive'})"
                @click="emit('update:page', pageNumber)"
            >
                {{ pageNumber }}
            </button>
            <button
                @click="props.page < pages ? emit('update:page', props.page + 1) : () => {}"
                :class="activeBtnClass({ size: 'md' })"
            >
                <span>Next</span><CustomIcons name="ArrowLeft" class="ml-1 w-6 h-6" />
            </button>
        </div>
    </div>
    </template>

<template>
    <div>
        ...
        <div class="flex flex-col items-center text-center">
          ...
          <DialogTitle class="text-xl font-bold text-[#09314F] mb-2">{{ title }}</DialogTitle>
          <DialogDescription class="text-gray-600 mb-6">
            {{ description }}
            <p class="mt-2">{{ subDescription }}</p>
          </DialogDescription>
          <Button @click="handleAction" class="w-40 bg-[#09314F] hover:bg-[#09314F] text-white rounded py-2">
            {{ buttonText }}
          </Button>
        </div>
      </DialogContent>

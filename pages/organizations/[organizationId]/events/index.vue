                ...
                >
                  <DropdownMenuItem :disabled="row.status === 'CANCELLED'">
                    <NuxtLink
                      ...
                      :to="`/organizations/${route.params.organizationId}/event/${row.id}`"
                    >
                      Ver Evento
...
const filterOptions = ref(
  `[{ "field": "organization.id", "type": "equal", "value": "${route.params.organizationId}" }]`,
);
...
const [eventListData, organizationSummaryData] = await Promise.all([
  ...
  useAPI<IOrganizationSummary>(`${BASE_ORG_URL}/get-events-summary`, {
    query: { organizationId: route.params.organizationId },
  } as any),
]);

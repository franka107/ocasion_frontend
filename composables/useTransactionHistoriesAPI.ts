...
export function useTransactionHistoriesAPI() {
  ...
  const getData = async () => {
    const [transactionHistoryList, transactionHistoryResume] = await Promise.all([
      ...
      useAPI<{ incomeAmount: number, penaltiesChargedAmount: number, withdrawalsPendingProcessingAmount: number}>(
  ...
  const handleExport = async () => {
    openConfirmModal({
      ...
      callback: async () => {
          const { data: response, error, status } = await useAPI<string | any>(`${BASE_TRANSACTION_URL}/export-transaction-histories?filterOptions=${filterOptions.value}`,{} as any)
